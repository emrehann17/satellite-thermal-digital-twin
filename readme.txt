...
- anomaly üretimine temel oluşturacak raster çıktılar elde etmek
...
- aynı tarihli çıktılar için kullanılan geçici indeksleme yaklaşımının kaldırılması
...
- `ee.Image.getDownloadURL()` ile doğrudan indirme yaklaşımı denenmiştir; ancak büyük boyutlu raster çıktılarında yeterli ve kararlı bir çözüm sunmamıştır.
- Bu nedenle mevcut aşamada ana akış için **Drive export + manuel indirme** yaklaşımı korunmaktadır.
...
- Üretilen anomaly raster şu an tüm bölgenin nihai anomaly’sini değil, kullanılan son dönem Landsat verisinin geçerli kapsama alanını temsil etmektedir.
- Anomaly kapsamını genişletmek amacıyla son tek sahne yerine son 3 çıktının ortalaması / birlikte değerlendirilmesi yaklaşımı denenmektedir; ancak bu yöntem henüz nihai çözüm olarak doğrulanmış değildir.
...
- çıktının daha dengeli ve yorumlanabilir hale getirilmesi
...
## Landsat Günlük Composite Yaklaşımı

Landsat verilerinde aynı tarihe ait birden fazla sahne oluşabildiği için, bu görüntüleri Step5 tarafında ayrı ayrı işlemek yerine Step3/Step4 düzeyinde tek günlük çıktıya indirgeme yaklaşımı tercih edilmiştir. Bu amaçla aynı güne ait sahneler **daily median composite** yöntemiyle birleştirilmektedir.

Bu tercih, aynı tarihli çoklu sahnelerin Step5 tarafında tekrar sorun üretmesini engellemek ve yerel raster işleme aşamasına daha temiz veri sağlamak için yapılmıştır. Böylece Step5’e her tarih için tek bir temsil edici Landsat çıktısı aktarılması hedeflenmektedir.

## Anomaly Çıktısı Hakkında Not

Projede anomaly raster üretimi çalıştırılmıştır; ancak bu çıktı şu aşamada nihai bir bölgesel anomaly haritası olarak değerlendirilmemelidir. İlk denemelerde az sayıda Landsat sahnesi kullanıldığı için bazı alanlarda veri boşlukları oluşmuştur. Bunun başlıca nedenleri, sınırlı sahne sayısı, QA tabanlı maskeleme ve sahne kapsama alanlarının farklılığıdır.

Mevcut yaklaşım, anomaly üretimini teknik olarak mümkün kılan bir ilk prototip düzeyindedir. Sonraki aşamada daha fazla Landsat sahnesi kullanılarak anomaly kapsamının genişletilmesi ve çıktının tüm bölgeyi daha iyi temsil edecek şekilde iyileştirilmesi hedeflenmektedir.
