...
| `zamora_2022` | Disabled placeholder | Registry'de kayıt var | — | — | Canonical dataset yok | Çalıştırılabilir kanıt değildir |

Kaynaklar: registry ve tarihler `core/regions.py`; cell/population counts `outputs/step8a/step8a_dataset_stats.json`, `outputs/experiments/manavgat_2021/step8a/step8a_dataset_stats.json`, `outputs/experiments/bejis_2022/step8a/step8a_dataset_stats.json`; land-cover gate `outputs/step6/labels/burned_landcover_gate.md` ve experiment Step8A stats. `core/regions.py` içinde bir `valencia_2022_aoi` geometry helper bulunur, ancak `EXPERIMENTS` registry'sinde Valencia experiment yoktur. `docs/experiments.md` bunu disabled experiment gibi gösterir; çalıştırılabilirlik için code registry doğrudur.
